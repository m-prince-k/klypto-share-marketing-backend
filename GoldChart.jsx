    ...
    useEffect(() => {
            ...
            try {
                if (payload.success && payload.data?.length > 0) {
                    ...
                    const formattedData = payload.data.map(c => {
                        ...
                        if (isNaN(c.time)) return false;
                        if (selectedSymbol === 'GOLD') return c.close > 50000;
                        return c.close > 0;
        ...
        socket.on(EVENTS.LIVE_TICK, (tick) => {
            ...
            const isMatch = tick.symbol === selectedSymbol ||
                ((selectedSymbol === 'GOLD' || selectedSymbol === 'SILVER') && tick.symbol.startsWith(selectedSymbol));

            if (isMatch) {
                const currentPrice = ltp > 0 ? ltp : (tick.data ? tick.data.close : 0);

                // --- PRICE SAFETY FILTERS ---
                if (currentPrice < 100) return; 
                if (selectedSymbol === 'GOLD' && currentPrice < 50000) return;
                // ----------------------------

                const istOffset = 5.5 * 60 * 60;
                let tickTime = Math.floor(Date.now() / 1000);
                
                if (tick.data && tick.data.time) {
                    tickTime = Number(tick.data.time);
                    if (tickTime > 10000000000) tickTime = Math.floor(tickTime / 1000);
                }

                // USE ONLY currentPrice (LTP) for live candles
                const tickData = {
                    time: tickTime + istOffset,
                    open: currentPrice, 
                    high: currentPrice, 
                    low: currentPrice, 
                    close: currentPrice
        ...
        socket.on(EVENTS.GOLD_UPDATE, (tick) => {
            const istOffset = 5.5 * 60 * 60;
            if (selectedSymbol === 'GOLD' && tick.data && tick.data.close > 50000) {
                const tickData = { ...tick.data };
                if (tickData.time < 10000000000) tickData.time = tickData.time + istOffset;
                else tickData.time = Math.floor(tickData.time / 1000) + istOffset;
                updateChart(tickData);
            }
        });
            ...
            <aside className="w-72 border-r border-slate-800/50 flex flex-col bg-slate-900/20 backdrop-blur-xl z-20">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                    {stocks
                        ...
                        .map((s, idx) => (
                            <div key={idx} onClick={() => setSelectedSymbol(s.name)} className={`p-4 rounded-4 text-black cursor-pointer transition-all mb-2 ${selectedSymbol === s.name ? 'bg-primary bg-opacity-10 border border-primary border-opacity-30' : 'bg-slate-900/40 border border-slate-800/50 hover-bg-slate-800'}`}>
                ...
                <header className="h-20 border-b border-slate-800/50 d-flex align-items-center justify-content-between px-6 bg-slate-900 bg-opacity-40 backdrop-blur-md">
                    <div className="d-flex align-items-center gap-5">
                        <div className="d-flex flex-column gap-1">
                            ...
                            >
                                {stocks.map((s, idx) => (
                                    <option key={idx} value={s.name} className="bg-slate-900 text-white">{s.name}</option>
                                ))}
                            </select>
