    ...
    try {
        ...
        if (dbCandles.length > 0) {
            ...
            const intervalInMinutes = {
                ...
                "ONE_HOUR": 60, "ONE_DAY": 375
            }[interval] || 1;

            const marketHoursPerDay = 6.25; 
            const expectedCandlesPerDay = (marketHoursPerDay * 60) / intervalInMinutes;
            const expectedCount = Math.max(1, Math.floor(rangeDays * expectedCandlesPerDay) * 0.8);

            if (dbCandles.length >= expectedCount || (rangeDays < 0.1 && dbCandles.length > 0)) {
                console.log(`[DB Cache] Serving ${dbCandles.length} records from ${ModelToUse.name} for ${symbol}`);
                return { 
                    source: "database", 
        ...
        while (currentStartDate < finalEndDate) {
            ...
            if (response && response.status && response.data) {
                ...
                console.log(`[API Chunk] ${symbol} Empty or Error: ${response?.message || 'No message'}`);
            }
