            ...
            try {
                ...
                if (response && response.status && response.data && response.data.fetched) {
                    ...
                    if (response && (String(response.status) === "403" || response.errorcode === "AB1004" || String(response.message).includes("Invalid Token"))) {
                        console.log(`[LiveSync] Token Expired! Forcing Re-Login...`);
                        const { login } = require('./authService');
                        await login(true);
                        
                        console.log(`[LiveSync] Retrying chunk...`);
                        const retryResponse = await smartApi.marketData({ mode: "FULL", exchangeTokens: exchangeTokens });
                        if (retryResponse && retryResponse.status && retryResponse.data && retryResponse.data.fetched) {
                            console.log(`[LiveSync] Retry Successful!`);
                            // Let the next loop iteration handle it properly, or just ignore for this 1-second interval to avoid code duplication
                        }
                    }
                }
            } catch (chunkError) {
