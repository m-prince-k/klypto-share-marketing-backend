...
function calculateSSL(data) {
    const requiredCols = [
        ...
        "exitHigh", "exitLow",
        "Baseline",
        "ATR", "ATR_Upper", "ATR_Lower",
        "HLV1", "HLV2", "HLV3",
        "SSL_Line", "SSL_Trend",
        "SSL2_Line", "SSL2_Trend",
        "SSL_Exit"
    ];
    ...
    if (needsInit) {
        ...
        const emaHigh = HMA(data.map(x => x.high), 60);
        const emaLow = HMA(data.map(x => x.low), 60);

        const maHigh2 = HMA(data.map(x => x.high), 5);
        const maLow2 = HMA(data.map(x => x.low), 5);

        const exitHigh = HMA(data.map(x => x.high), 15);
        const exitLow = HMA(data.map(x => x.low), 15);

        const baseline = HMA(data.map(x => x.close), 60);
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (i === 13) {
                ...
                data[i].ATR =
                    ((data[i - 1].ATR * 13) + trList[i]) / 14;
            }

            data[i].ATR_Upper =
                data[i].close + data[i].ATR;
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (c > data[i].maHigh2)
                prev2 = 1;
            else if (c < data[i].maLow2)
                prev2 = -1;

            data[i].HLV2 = prev2;

            // SSL3
            if (c > data[i].exitHigh)
                prev3 = 1;
            else if (c < data[i].exitLow)
                prev3 = -1;
    ...
    // ==================================
        ...
        data[curr].emaHigh =
            HMA(data.map(x => x.high), 60).at(-1);

        data[curr].emaLow =
            HMA(data.map(x => x.low), 60).at(-1);

        data[curr].maHigh2 =
            HMA(data.map(x => x.high), 5).at(-1);

        data[curr].maLow2 =
            HMA(data.map(x => x.low), 5).at(-1);

        data[curr].exitHigh =
            HMA(data.map(x => x.high), 15).at(-1);

        data[curr].exitLow =
            HMA(data.map(x => x.low), 15).at(-1);

        data[curr].Baseline =
            HMA(data.map(x => x.close), 60).at(-1);
        ...
        data[curr].ATR =
            ((data[prev].ATR * 13) + tr) / 14;

        data[curr].ATR_Upper =
            data[curr].close + data[curr].ATR;
...
async function generateBoslim(rawData = null) {
    let boslim;
    ...
    const fmt = (val) => Number.isNaN(val) || val == null ? "NaN" : val.toString();
    ...
    boslim = boslim.map((row, index) => {
        ...
        return {
            ...
            Gain: fmt(row.Gain),
            Loss: fmt(row.Loss),
            "Avg Gain": fmt(row.Avg_Gain),
            "Avg Loss": fmt(row.Avg_Loss),
